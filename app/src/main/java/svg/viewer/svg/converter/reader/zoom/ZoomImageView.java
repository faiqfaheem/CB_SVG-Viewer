package svg.viewer.svg.converter.reader.zoom;
...
public class ZoomImageView extends AppCompatImageView implements OnScaleGestureListener {
    ...
    private boolean translatable, zoomable, doubleTapToZoom, restrictBounds, animateOnReset, autoCenter;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.iv);

        zoomable = values.getBoolean(R.styleable.iv_zoomable, true);
        translatable = values.getBoolean(R.styleable.iv_translatable, true);
        animateOnReset = values.getBoolean(R.styleable.iv_animateOnReset, true);
        autoCenter = values.getBoolean(R.styleable.iv_autoCenter, true);
        restrictBounds = values.getBoolean(R.styleable.iv_restrictBounds, false);
        doubleTapToZoom = values.getBoolean(R.styleable.iv_doubleTapToZoom, true);
        minScale = values.getFloat(R.styleable.iv_minScale, MIN_SCALE);
        maxScale = values.getFloat(R.styleable.iv_maxScale, MAX_SCALE);
        doubleTapToZoomScaleFactor = values.getFloat(R.styleable.iv_doubleTapToZoomScaleFactor, 3);
        autoResetMode = AutoResetMode.Parser.fromInt(values.getInt(R.styleable.iv_autoResetMode, AutoResetMode.UNDER));
        verifyScaleRange();
        values.recycle();
    }
